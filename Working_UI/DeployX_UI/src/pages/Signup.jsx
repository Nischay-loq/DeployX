...
export default function Signup() {
  ...
  const [isGoogleLoading, setGoogleLoading] = useState(false); // 🔹 New state
  ...
  const handleSubmit = async (e) => {
    ...
    setTimeout(() => {
      ...
      const storedUsers = JSON.parse(
        localStorage.getItem("deployx_users") || "[]"
      );
      const newUser = {
        id: Date.now(),
  ...
  const handleGoogleSignup = async () => {
    if (isGoogleLoading) return; // 🔹 Prevents double clicks
    ...
    try {
      ...
      setGoogleLoading(false); // 🔹 Re-enable button
    }
  };
  ...
  return (
    <div className="min-h-screen flex items-center justify-center relative overflow-hidden bg-[radial-gradient(circle_at_20%_20%,rgba(0,255,247,0.12),transparent_35%),radial-gradient(circle_at_80%_80%,rgba(0,168,255,0.12),transparent_35%)]">
      ...
      <div className="absolute top-6 left-6 z-10">
        <Link
          to="/"
          className="px-4 py-2 border border-electricBlue rounded-lg text-electricBlue hover:bg-electricBlue hover:text-cyberBlue transition-all cursor-pointer"
        >
          ← Back to Home
        </Link>
      </div>
      ...
      <div className="absolute inset-0 flex items-center justify-center p-6 form-container">
        <div className="glass-light border-trace rounded-3xl w-full max-w-md p-8">
          {showSuccess ? (
            ...
            <>
              <h1 className="text-3xl font-bold mb-6 text-center text-softWhite">
                Create your DeployX account
              </h1>
              <form className="space-y-4" onSubmit={handleSubmit}>
                <div>
                  <label className="block text-sm mb-1 text-softWhite cursor-pointer">
                    Full Name
                  </label>
                  <input
                    name="fullName"
                ...
                <button
                  ...
                  className={`w-full px-6 py-3 rounded-xl flex items-center justify-center gap-3 font-semibold transition-all ${
                    isGoogleLoading
